...
const sampleStudents: StudentSeed[] = [
  ...
  {
    email: "aiml.student@college.edu",
  ...
  branches.forEach((branch) => {
    (Object.entries(semesterCourses) as Array<[string, (typeof semesterCourses)[1]]>).forEach(
      ([semesterKey, courses], index) => {
        ...
        const facultyId = facultyIds[(index + branches.indexOf(branch)) % facultyIds.length];
...
async function seedData() {
  ...
  await executeMany(
    `
      INSERT INTO course_offering (academic_year, semester, branch, course_id, faculty_id)
      VALUES (:academic_year, :semester, :branch, :course_id, :faculty_id)
    `,
    offeringRows,
  );
