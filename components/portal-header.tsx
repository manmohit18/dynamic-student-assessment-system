...
export function PortalHeader({ name, role }: PortalHeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-white/10 bg-slate-950/80 backdrop-blur-2xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-4">
          <div className="grid h-11 w-11 place-items-center rounded-2xl bg-amber-400 text-slate-950 shadow-[0_12px_36px_rgba(251,191,36,0.35)]">
            <ShieldCheck className="h-5 w-5" />
          </div>
          <div>
            <p className="text-xs uppercase tracking-[0.32em] text-slate-400">Course portal</p>
            <h1 className="text-lg font-semibold text-white">{name}</h1>
          </div>
        </div>

        <nav className="hidden items-center gap-2 md:flex">
          <Link href="/" className="rounded-full px-4 py-2 text-sm text-slate-200 hover:bg-white/5">
          ...
          {role !== "student" ? (
            <Link href="/faculty" className="rounded-full px-4 py-2 text-sm text-slate-200 hover:bg-white/5">
              Faculty
            </Link>
          ) : null}
        </nav>
